...
@Client.on_message(
    command(["stop", "قف", "يقاف", f"stop@{BOT_USERNAME}", "end", f"end@{BOT_USERNAME}", "vstop"])
    & other_filters
)
@authorized_users_only
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply("✅ **تم ايقاف التشغيل يقلبي**")
...
@Client.on_message(
    command(["pause", "سكت", f"pause@{BOT_USERNAME}", "vpause"]) & other_filters
)
@authorized_users_only
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        ...
        await m.reply("❌ **قائمة التشغيل فارغه يقلبي**")


@Client.on_message(
    command(["resume", "مل", f"resume@{BOT_USERNAME}", "vresume"]) & other_filters
)
@authorized_users_only
...
async def cbpause(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        ...
        await query.answer("💛🙂 مفيش اغنيه يحب", show_alert=True)

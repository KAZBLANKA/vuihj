                ...
                if amort == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        try:
                            ...
                            await suhu.delete()
                            await m.reply_text(f"خطاء قام احد مشرفي المجموعه بحظر الحساب المساعد برجاء رفع الحظر عنه لكي اعمل @U_And_rold : `{ep}`")


# stream is used for live streaming only


@Client.on_message(command(["stream","play","شغيل","عل",f"stream@{BOT_USERNAME}"]) & other_filters)
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
              ...
              [InlineKeyboardButton("أضف لبوت لمجموعتك ✅", url=f"http://t.me/U_And_ro_dbot?startgroup=new"),               
            ]
        ]
    )
    if m.sender_chat:
